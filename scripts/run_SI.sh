...
# exp -t ${task} -n ${name} -s ${step} --feat ard -gamma 1.0 --inc --dist_type id -alpha 0.5 -beta 1.0 -mb 2000 -mt mean -cvd 0
#name=Finetune
#exp -t ${task} -n ${name} -s ${step} -cvd 0,1
...
# exp -t ${task} -n ${name} -s ${step} --feat ard -gamma 1.0 --inc --dist_type id -alpha 0.1 -beta 0.5 -mb 2000 -mt mean -cvd 0
# name=Finetune
#exp -t ${task} -n ${name} -s ${step} -cvd 0,1
...
#task=5-15
#name=ABR_LR005_BS4_ALPHA01_BETA05_GAMMA1
#exp -t ${task} -n ${name} -s ${step} --feat afd -gamma 1.0 --uce --dist_type id -alpha 0.1 -beta 0.5 -mb 2000 -mt mean -cvd 0,1
#name=Finetune
#exp -t ${task} -n ${name} -s ${step} -cvd 0,1
