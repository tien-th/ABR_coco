...
_C.MODEL.SOURCE_WEIGHT = ""  # no pre-trained model
...
# probability of train image flipping
_C.INPUT.FLIP_PROB_TRAIN = 0.5  # horizontally flip the image in percentage of 0.5
...
_C.MODEL.BACKBONE.FREEZE_CONV_BODY_AT = 2  # 0 means no layer is frozen, 2 means frozen first 2 stages
# GN for backbone
_C.MODEL.BACKBONE.USE_GN = False
# Whether freeze the whole backbone network
_C.MODEL.BACKBONE.ALL_FREEZE = False
# Whether freeze the FPN network
_C.MODEL.BACKBONE.FPN_FREEZE = False
...
# Whether use external proposal generate to generate proposals
_C.MODEL.RPN.EXTERNAL_PROPOSAL = False
# Whether freeze the RPN convolutional layer
_C.MODEL.RPN.CONV_FREEZE = False
# Whether freeze the RPN classification layer
...
# Whether freeze the 2 Fully Connected Layers within the ROI sub-network
_C.MODEL.ROI_HEADS.FC_FREEZE = False
# Whether freeze the Classification Layer within the ROI sub-network
_C.MODEL.ROI_HEADS.CLS_FREEZE = False
# Whether freeze the Bounding Box Regression Layer within the ROI sub-network
_C.MODEL.ROI_HEADS.BBS_FREEZE = False
# Whether add offset to ROI sub-network Classification Layer
_C.MODEL.ROI_HEADS.CLS_OFFSET = False
# Whether add offset to ROI sub-network Bounding Box Regression Layer
_C.MODEL.ROI_HEADS.BBS_OFFSET = False
...
_C.MODEL.ROI_BOX_HEAD.NAME_OLD_CLASSES = []
...
_C.SOLVER.GAMMA = 0.1  # decay rate
...
_C.TEST.COCO_ALPHABETICAL_ORDER = True
...
_C.TENSORBOARD_DIR = "./tensorboardx"
